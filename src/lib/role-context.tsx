...
function formatSince(createdAt: string | null): string {
  if (!createdAt) return "";
  const d = new Date(createdAt);
  if (isNaN(d.getTime())) return "";
  return `${MESES_FOR_SINCE[d.getMonth()]} ${d.getFullYear()}`;
}
...
export function RoleProvider({ children }: { children: ReactNode }) {
  ...
  const fetchProfile = useCallback(async () => {
    ...
    const { data: profileData } = await supabase
      .from("profiles")
      .select("name, phone, org, title, bio, location, avatar_url, created_at")
      .eq("user_id", user!.id)
      .maybeSingle();
    ...
    if (profileData) {
      setProfile({
        ...
        location: profileData.location || "Barranquilla, Colombia",
        since: formatSince(profileData.created_at),
        bio: profileData.bio || "",
  ...
  const value = useMemo<RoleContextValue>(
    ...
    [role, profile, profileLoading, fetchProfile],
  );
