...
- `src/routes/app/clients.$id.tsx` — client detail page that queries **real Supabase data** (client row + their appointments). All rows in README marked "Conectado" are real now.
- `src/routes/app/appointments.tsx` — **two traps**: (1) PostgREST caps un-ranged queries at 1000 rows, so the appointments fetch **paginates with `.range(from, from+999)`** (loop until a page < 1000) — don't revert to a single unbounded query or newly-created citas disappear from the calendar; (2) the `time` column is Postgres `time` → comes back as `"HH:MM:SS"`, so it's **normalized with `a.time.slice(0, 5)`** on fetch; calendar slots compare `"HH:MM"` and otherwise no appointment ever renders in the week/day grid.
